...
`TObjectPtr<T>`는 UObject를 가리키는 멤버 포인터라는 의도를 엔진에 더 명확하게 알려주는 래퍼 타입이다.

겉으로는 일반 포인터처럼 `CachedObject->DoSomething()` 형태로 사용하지만, 내부적으로는 Unreal의 객체 핸들 시스템과 더 잘 맞게 설계되어 있다.
