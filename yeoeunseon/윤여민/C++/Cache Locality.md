...
```mermaid
flowchart TD
    subgraph Vector["std::vector (연속 메모리)"]
        direction LR
        V1["[1] (4B)"] --- V2["[2] (4B)"] --- V3["[3] (4B)"] --- V4["[4] (4B)"]
        V1 & V2 & V3 & V4 --> CL1["한 캐시 라인에 함께 적재될 수 있음"]
    end

    subgraph List["std::list (비연속 메모리)"]
        direction LR
        L1["[Node 1]"] -.->|Next Pointer| L2["[Node 2]"]
        L2 -.->|Next Pointer| L3["[Node 3]"]
        L3 -.->|Next Pointer| L4["[Node 4]"]
        L1 --> CL2["노드 주변 데이터가 함께 적재됨"]
    end

    style CL1 fill:#d6ffd6
    style CL2 fill:#ffd6d6
```
...
CPU가 요청한 데이터가 현재 확인하는 캐시 계층에 없는 상태다. 이 경우 다음 캐시 계층이나 메인 메모리에서 데이터를 가져와야 하므로 Cache Hit보다 지연 시간이 길어진다. 모든 Cache Miss가 곧바로 RAM 접근을 뜻하는 것은 아니다.
...
반복자로 삽입·삭제 위치를 이미 알고 있고 중간 삽입·삭제가 매우 빈번하거나, 삭제되지 않은 요소의 반복자와 참조를 유지해야 한다면 `list`를 검토할 수 있다. 양 끝 삽입·삭제가 잦고 임의 접근도 필요하다면 `std::deque`가 후보가 될 수 있다.

요소가 크면 `vector`의 중간 삽입·삭제에서 큰 객체를 여러 개 이동하는 비용이 커질 수 있다. 그렇다고 바로 `list`가 더 빠른 것은 아니다. 프로그램이 삽입·삭제보다 전체 요소를 순회하는 일을 더 자주 한다면, 연속 메모리를 사용하는 `vector`가 여전히 유리할 수 있기 때문이다.

예를 들어 적 1,000개를 한 번 추가한 뒤 매 프레임 모두 갱신한다면, 중요한 작업은 삽입보다 반복 순회다. 이 경우 객체가 조금 크더라도 `vector` 또는 `TArray`가 적합할 가능성이 높다. 반대로 중간 요소를 계속 삽입·삭제하고 순회는 거의 하지 않는다면 다른 컨테이너를 검토할 수 있다.
...
`vector`와 `list`의 전체 순회는 모두 Big-O로 `O(n)`이다. 하지만 `vector`는 요소가 붙어 있어 캐시 라인과 프리페칭을 효율적으로 사용할 수 있고, `list`는 다음 노드의 주소를 따라가야 한다. 따라서 같은 `O(n)`이어도 실제 실행 시간은 다를 수 있다.

컨테이너를 선택할 때는 다음 순서로 생각하면 이해하기 쉽다.

1. 가장 자주 하는 작업이 순회인지, 검색인지, 삽입·삭제인지 확인한다.
2. 데이터가 연속으로 배치될 때 캐시의 이점을 얻을 수 있는지 확인한다.
3. 반복자나 포인터의 주소가 계속 유지되어야 하는지 확인한다.
4. 후보 컨테이너를 실제 데이터 규모로 측정한다.
